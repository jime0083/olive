...
**各セクション同士の間の空白は常に同じ大きさに揃えること。要素を増減しても余白が崩れてはならない。**

- 余白は `src/App.css` の `:root` で定義した CSS 変数で一元管理する:
  - `--section-space-y`（上下の余白, PC: 80px / ≤900px: 48px）
  - `--section-space-x`（左右の余白, PC: 10% / ≤900px: 5%）
- 各コンテンツセクション（story / policy / about / media 等）の縦横パディングは
  **必ず `padding: var(--section-space-y) var(--section-space-x);` を使う**。
  個別のpx値をハードコードしない。
- 上下パディングを同じ変数にすることで、セクションの中身（要素数）が変わっても
  セクション間の余白は常に一定になる。
- 余白の大きさを変えたいときは、各セクションではなく **`:root` の変数値のみ** を変更する。
- レスポンシブでの余白調整も、個別セクションではなく `@media` 内の `:root` 変数で行う。

⛔ **新しいセクションを追加する場合も、必ず上記の変数を使って余白を付けること。**
